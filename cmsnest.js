...
console.log("conflict mac2 MACBOOK.");
...
(() => {
    ...
    var O = (e = document) => {
        ...
        for (let n of e.childNodes) if (n.nodeType === Node.COMMENT_NODE && ((o = n.textContent) != null && o.includes(t))) {
            let r = n.textContent.trim().split(t)[1];

            console.log(" line68: r = ", r);
    ...
    var St = N(H, "1"), Q = async () => {
        let {fsAttributes: e} = window;
        e.cmscore || (e.cmscore = {});
        let {cmscore: t} = e;
        if (t.import) return t.import;
        try {
            return t.import = import(St), t.import.then(o => {
    ...
    var gt = `${A}-${M}`, Z = async () => {
        var r;
        let {fsAttributes: e, location: t} = window, {host: o, searchParams: n} = new URL(t.href);
        return !o.includes("webflow.io") || !n.has(gt) ? !1 : (r = e.import) == null ? void 0 : r.call(e, M, "1")
    };
    var L = e => {
        let t = (r, s, i) => {
            let c = e[r], {key: l, values: a} = c, u;
            if (!s) return `[${l}]`;
            let p = a == null ? void 0 : a[s];
            _(p) ? u = p : u = p(i && "instanceIndex" in i ? i.instanceIndex : void 0);
        ...
        function o(r, s) {
            let i = t("element", r, s), c = (s == null ? void 0 : s.scope) || document;
    ...
    var tt = e => {
        ...
        let r = {preventsLoad: _(t.getAttribute(C.preventLoad.key)), attributes: o};
        for (let s in e) {
    ...
    var et = ({scriptAttributes: e, attributeKey: t, version: o, init: n}) => {
        ...
        let {preventsLoad: r, attributes: s} = tt(e), i = window.fsAttributes[t];
        ...
        let t = At(e);
        $(t, e), yt(t), window.fsAttributes = t, window.FsAttributes = window.fsAttributes, Z()
    }, At = e => {
        let t = {
            cms: {}, push(...o) {
                var n, r;
                ...
                return r || new Promise(s => {
                    ...
                    i.src = N(o, n), i.async = !0, i.onload = () => {
                        let [c] = $(t, [o]);
                        s(c)
        ...
        let e = D("src", "finsweet", {operator: "contains"}), t = D("dev");
        return [...document.querySelectorAll(`script${e}, script${t}`)].reduce((r, s) => {
            var c;
            let i = s.getAttribute(C.dev.key) || ((c = s.src.match(/[\w-. ]+(?=(\.js)$)/)) == null ? void 0 : c[0]);
            return i && !r.includes(i) && r.push(i), r
        }, [])
    }, $ = (e, t) => t.map(n => {
        let r = e[n];
        return r || (e[n] = {}, r = e[n], r.loading = new Promise(s => {
            r.resolve = i => {
    ...
    function P(e, t, o = document) {
        ...
        return t === "wrapper" ? r : t === "list" ? s : t === "items" ? [...(s == null ? void 0 : s.querySelectorAll(`:scope > ${d("item")}`)) || []] : t === "pageCount" ? r.querySelector(d("pageCount")) : t === "empty" ? r.querySelector(`:scope > ${d("emptyState")}`) : t === "pagination" ? r.querySelector(d("paginationWrapper")) : r.querySelector(d(t === "next" ? "paginationNext" : "paginationPrevious"))
    }

    var b = "pages", k = new Map,
        st = async (e, {cache: t = !0, cacheExternal: o, cacheKey: n, cacheVersion: r} = {}) => {
            var s, i;
            try {
                let c = new URL(e, window.location.origin), l = await _t(c);
                if (l) return l;
                let a = h(), u = O(), p = a || n,
                    m = (i = (s = u == null ? void 0 : u.getTime()) != null ? s : r) != null ? i : 1,
            ...
            let {page: r, rawPage: s} = await it(t), i = ct(r, o);
            return !i && !n || (await Nt(e, t.href, s), i && k.delete(t.href)), r
        }, ct = (e, t) => {
            if (!t) return !1;
            let o = h(e);
            return o && o === t
        }, K = e => new DOMParser().parseFromString(e, "text/html"), Rt = (e, t) => new Promise(o => {
            try {
                ...
                n.onblocked = () => {
                    ...
                    let r = n.result;
                    r.objectStoreNames.contains(b) && r.deleteObjectStore(b), r.createObjectStore(b)
                }, n.onerror = () => o(null), n.onsuccess = () => {
                    let r = n.result;
    ...
    var B = `fs-${x}`, Lt = "list", Pt = "nest-target", Bt = "slugs", Mt = "collection", Ut = "empty", vt = "cache",
        Ot = {false: "false"}, E = {
            element: {key: `${B}-element`, values: {list: Lt, nestTarget: Pt, slugs: Bt}},
            collection: {key: `${B}-${Mt}`},
            empty: {key: `${B}-${Ut}`},
            cacheItems: {key: `${B}-${vt}`, values: Ot}
        }, [S, Fe, at] = L(E);
    var lt = ({createCMSListInstances: e}) => {
        let t = new Map, o = e([S("collection")]);
        for (let n of o) {
            let r = w(n.getAttribute(E.collection.key));
            if (!r) continue;
            let s = document.querySelector(`[${E.empty.key}^="${r}"]`);
            s && (s.style.display = "none"), n.wrapper.style.display = "none", t.set(r, {
                listInstance: n,
                emptyElement: s
            })
        }
        ...
        let o = [...e.querySelectorAll(`${S("collection")}:not(a)`)].reduce((s, i) => {
            let c = w(at(i, "collection"));
            if (!c) return s;
            let l = s.get(c) || [];
            return l.push(i), s.set(c, l), s
        }, new Map), n = new Map, r = new Map;
        for (let [s, i] of o) {
    ...
    var I = async (e, t, o, n) => {
    ...
    }, Dt = (e, t, o, n) => Promise.all([...e].map(([r, {nestTarget: s, slugs: i}]) => {
        let c = t.get(r);
        if (!c) return;
        let l = c.listInstance.items.filter(({href: a}) => {
            if (!a) return !1;
            try {
                let u = new URL(a), [p] = u.pathname.match(/[^/]+(?=\/$|$)/g) || [];
        ...
        if (!i) return;
        let c = rt([S("collection")], i), l = new Set;
        await Promise.all(c.map(async (a, u) => {
            let p = new s(a, u), m = w(p.getAttribute(E.collection.key));
            if (!m || l.has(m)) return;
            l.add(m);
            let f = o.get(m), g = e.get(m);
            if (!f || !g) return;
        ...
        if (!l) return;
        let a = R(o.wrapper), u = P(a, "items");
        if (a.style.display = "", t.length) await Promise.all([...u].map(p => {
    ...
    var ft = async (e, t) => {
        let o = lt(t);
        if (!o.size) return;
        mt(e, o, t), e.getAttribute(E.cacheItems.key) === E.cacheItems.values.false && (e.cacheItems = !1);
        let r = [...e.items];
        await Promise.all(r.map(s => I(s, o, e.cacheItems, t))), await e.emitSerial("nestinitialitems", r)
    };
    var Tt = async () => {
        let e = await Q();
        if (!e) return [];
        ...
        return await Promise.all(t.map(o => ft(o, e))), J(x, t, () => {
